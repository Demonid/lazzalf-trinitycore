...
bool Player::TeleportTo(uint32 mapid, float x, float y, float z, float orientation, uint32 options)
{
    ...
    if (GetMapId() == mapid) //if ((GetMapId() == mapid && !m_transport) || (GetTransport() && GetMapId() == 628))
    {
    ...
    else
    {
        ...
        */
        ...
        Map* map = sMapMgr->CreateMap(mapid, this, 0); //  Map* map = sMapMgr->FindMap(mapid, instanceId); //Map *map = sMapMgr->FindMap(mapid);
...
{
    GetAntiCheat()->SetSleep(3000); //AntiCheat Sleep
