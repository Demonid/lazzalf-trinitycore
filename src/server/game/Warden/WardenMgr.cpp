...
{
    m_HalfCall = !m_HalfCall;
    if (!m_HalfCall)
        return;

    if (session->m_WardenTimer.Passed())
    {
        switch (session->m_wardenStatus)
        {
            case WARD_STATE_UNREGISTERED:
                ...
                return;
            case WARD_STATE_LOAD_MODULE:
            case WARD_STATE_LOAD_FAILED:
...
void WardenMgr::SendSeedAndComputeKeys(WorldSession* const session)
{
    ...
    memcpy(session->m_wardenSeed, s.AsByteArray(16), 16);
