...
WardenMgr::WardenMgr() : m_Disconnected(false), m_HalfCall(false)
{
...
void WardenMgr::Initialize(const char *addr, u_short port)
{
    ...
    if (session->m_WardenTimer.Passed())        // We don't care the connection to wardend state to do cheat-checks or register
    {
        switch (session->m_wardenStatus)
        {
            ...
            case WARD_STATE_LOAD_FAILED:       // no reply after we sent the module to client (20 secs)
                sLog->outCheat("Warden Manager: no reply received for module load or 2 times load failed, kicking account %u", session->GetAccountId());
                if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
                    session->KickPlayer();
                return;
            case WARD_STATE_TRANSFORM_SEED:    // no reply to transformed seed (20 secs)
                sLog->outCheat("Warden Manager: no transformed seed received, kicking account %u", session->GetAccountId());
                if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
                    session->KickPlayer();
                return;
            case WARD_STATE_CHEAT_CHECK_OUT:   // timeout waiting for a cheat check reply
                sLog->outCheat("Warden Manager: no Cheat-check reply received, kicking account %u", session->GetAccountId());
                if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
                    session->KickPlayer();
                return;
            case WARD_STATE_FORCE_CHEAT_CHECK_OUT:   // timeout waiting for a Force cheat check reply
                sLog->outCheat("Warden Manager: no Force Cheat-check reply received, kicking account %u", session->GetAccountId());
                if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
                    session->KickPlayer();
                return;
...
bool WardenMgr::ValidateTSeed(WorldSession* const session, const uint8 *codedClientTSeed)
{
    ...
    if (memcmp(&codedServerTSeed[0], codedClientTSeed, 20))
    {
        ...
        sLog->outCheat("Warden Manager: no ValidateTSeed, kicking account %u", session->GetAccountId());
        if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
            session->KickPlayer();        
        return false;
    }
...
bool WardenMgr::ValidateCheatCheckResult(WorldSession* const session, WorldPacket& clientPacket)
{
    ...
    {
        sLog->outCheat("Warden Cheat-check: Kicking account %u for failed check, Packet Checksum 0x%08X is invalid!", session->GetAccountId(), checksum);
        //ReactToCheatCheckResult(session, false);
        if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
            session->KickPlayer();
        return false;
        ...
        switch ((*checkList)[i].check)
        {
            ...
            case WARD_CHECK_MEMORY:
            {
                ...
                else
                {
                    ...
                    if (memcmp(&memContent[0], &(*checkList)[i].mem->Result[0], (*checkList)[i].mem->Length))
                    {
                        ...
                        sLog->outCheat("Kicking account %u for failed check, MEM Offset 0x%04X length %u has content '%s' instead of '%s'",
            ...
            case WARD_CHECK_FILE:
            {
                ...
                else
                {
                    ...
                    if (res || memcmp(resSHA1, (*checkList)[i].file->SHA, 20))
                    {
                        ...
                        sLog->outCheat("Kicking account %u for failed check, MPQ '%s' SHA1 is '%s' instead of '%s'", accountId, (*checkList)[i].file->String.c_str(), strResSHA1.c_str(), strReqSHA1.c_str());
                    }
                    pktLen = pktLen - 21;
                }
            ...
            case WARD_CHECK_LUA:
            {
                ...
                if (foundLuaLen > 0)
                {
                    ...
                    sLog->outCheat("Kicking account %u for failed check, Lua '%s' found as '%s'", accountId, (*checkList)[i].lua->String.c_str(), (char*)luaStr);
                    localCheck = false;
                    free(luaStr);
                }
            ...
            case WARD_CHECK_DRIVER:
            {
                ...
                if (res != 0xE9)
                {
                    if ((*checkList)[i].check == WARD_CHECK_DRIVER)
                        sLog->outCheat("Kicking account %u for failed driver check '%s'", accountId ,(*checkList)[i].driver->String.c_str());
                    else
                        sLog->outCheat("Kicking account %u for failed page check Offset 0x%08X, length %u", accountId, (*checkList)[i].page->Offset, (*checkList)[i].page->Length);
                    localCheck = false;
                }
...
void WardenMgr::ReactToCheatCheckResult(WorldSession* const session, bool result)
{
    ...
    {
        if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE) == 2)
        {
            std::string sText = ("Warden: " + std::string(session->GetPlayerName()) + " ha usato cheat ed è stato bannato");
            sWorld->SendGMText(LANG_GM_BROADCAST, sText.c_str());
            sWorld->BanAccount(session, 24 * HOUR, "Cheating software user", "Server guard");
        }
        else if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
            session->KickPlayer();
    }
}
    ...
    if (sWardenMgr->IsEnabled())
    {
        ...
        switch(warden_opcode)
        {
            case WARDC_MODULE_LOAD_FAILED:
                ...
                {
                    sLog->outCheat("Warden status WARD_STATE_LOAD_FAILED: Kicking account %u for failed check!", GetAccountId());
                    if (sWorld->getIntConfig(CONFIG_WARDEN_BAN_TYPE))
                        KickPlayer();
                }
                else
