...
BossSpellWorker::BossSpellWorker(ScriptedAI* bossAI)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Initializing BossSpellWorker object for boss %u difficulty %u",bossID,currentDifficulty);
    LoadSpellTable();
...
{
     sLog->outDebug(LOG_FILTER_TSCR, "BSW: Removing BossSpellWorker object for boss %u",bossID);
};
...
{
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Loading table of spells boss  %u difficulty %u", bossID , currentDifficulty);
    ...
    if (Result)
    {
        ...
        sLog->outDebug(LOG_FILTER_TSCR, "BSW: Loaded %u boss spell data records for boss %u", uiCount, bossID);
    }
    else
    {
...
CanCastResult BossSpellWorker::_BSWSpellSelector(uint8 m_uiSpellIdx, Unit* pTarget)
{
    ...
    Unit* pSummon = NULL;
        ...
        sLog->outDebug(LOG_FILTER_TSCR, "BSW: Casting spell number %u type %u",pSpell->m_uiSpellEntry[currentDifficulty], pSpell->m_CastTarget);
...
CanCastResult BossSpellWorker::_BSWCastOnTarget(Unit* pTarget, uint8 m_uiSpellIdx)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Casting (on target) spell number %u type %u",pSpell->m_uiSpellEntry[currentDifficulty], pSpell->m_CastTarget);
...
uint8 BossSpellWorker::FindSpellIDX(uint32 SpellID)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: spell %u not found  in boss %u spelltable. Memory or database error?", SpellID, bossID);
...
CanCastResult BossSpellWorker::_BSWDoCast(uint8 m_uiSpellIdx, Unit* pTarget)
{
    ...
    SpellTable* pSpell = &m_BossSpell[m_uiSpellIdx];
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Casting bugged spell number %u type %u",pSpell->m_uiSpellEntry[currentDifficulty], pSpell->m_CastTarget);
...
Unit* BossSpellWorker::_doSummon(uint8 m_uiSpellIdx, TempSummonType summontype, uint32 delay)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Summoning creature number %u type %u despawn delay %u",pSpell->m_uiSpellEntry[currentDifficulty], pSpell->m_CastTarget, delay);
...
Unit* BossSpellWorker::_doSummonAtPosition(uint8 m_uiSpellIdx, TempSummonType summontype, uint32 delay, float fPosX, float fPosY, float fPosZ)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Summoning creature number %u type %u despawn delay %u at position %f %f %f",pSpell->m_uiSpellEntry[currentDifficulty], pSpell->m_CastTarget, delay, fPosX, fPosY, fPosZ);
...
bool BossSpellWorker::_doRemove(uint8 m_uiSpellIdx, Unit* pTarget, SpellEffectIndex index)
{
    SpellTable* pSpell = &m_BossSpell[m_uiSpellIdx];
        ...
        sLog->outDebug(LOG_FILTER_TSCR, "BSW: Removing effect of spell %u type %u",pSpell->m_uiSpellEntry[currentDifficulty], pSpell->m_CastTarget);
...
CanCastResult BossSpellWorker::_DoCastSpellIfCan(Unit* pTarget, uint32 uiSpell, uint32 uiCastFlags, uint64 uiOriginalCasterGUID)
{
    ...
    if (!pCaster->IsNonMeleeSpellCasted(false) || (uiCastFlags & (CAST_TRIGGERED | CAST_INTERRUPT_PREVIOUS)))
    {
        ...
        {
            sLog->outDebug(LOG_FILTER_TSCR, "BSW: DoCastSpellIfCan by creature entry %u attempt to cast spell %u but spell does not exist.", boss->GetEntry(), uiSpell);
            return CAST_FAIL_OTHER;
        }
    }
    else
...
Unit*  BossSpellWorker::_SelectUnit(SelectAggroTarget target, uint32 uiPosition)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: Cannot find target for spell :(");
    return NULL;
}
...
Unit* BossSpellWorker::_doSelect(uint32 SpellID, bool spellsearchtype, float range)
{
    ...
    sLog->outDebug(LOG_FILTER_TSCR, "BSW: search result for random player, count = %u ",_count);
