 ...
 *
 * Copyright (C) 2010 MythCore <http://code.google.com/p/mythcore/>
 *
...
enum Says
{
    ...
    SAY_FIRST_TIME        = -1615175,

    SAY_AGGRO            = -1615176,

    SAY_SUMMONING_STAR    = -1615177,

    SAY_BIGBANG_1         = -1615178,
    SAY_BIGBANG_2         = -1615179,
    ...
    SAY_DESPAWN_1         = -1615184,
    SAY_DESPAWN_2         = -1615185,
    SAY_DESPAWN_3         = -1615186,

    SAY_DEFEATED_1        = -1615187,
    SAY_DEFEATED_2        = -1615188,
    SAY_DEFEATED_3        = -1615189,
    SAY_DEFEATED_4        = -1615190,
    SAY_DEFEATED_5        = -1615191
};
...
public:
    ...
    {
        boss_algalon_the_observerAI(Creature* pCreature) : BossAI(pCreature, BOSS_ALGALON)
        {
        ...
        void Reset()
        {
            ...
            for (ThreatList::const_iterator i = list.begin(); i != list.end(); ++i)
            {
                ...
                //if (pP->IsImmunedToSpell(bigbang))
                    pP->ApplySpellImmune(RAID_MODE(H_SPELL_BIG_BANG, SPELL_BIG_BANG), IMMUNITY_ID, 0, false);
            }
        }
        ...
        {
            if (Instance->GetBossState(BOSS_ALGALON) == NOT_STARTED)
            {
        ...
        void EnterCombat(Unit* pWho)
        {
            ...
            DoStartCombat();
            space = me->SummonGameObject(GO_SPACE, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0, 0, 0, 0, 0,3600000);
			space->SetGoState(GO_STATE_ACTIVE);
        ...
        void UpdateAI(const uint32 diff)
        {
            ...
            if (!bAscentToHeavens() && !DespawnStart)
            {
                ...
                //        pU->ApplySpellImmune(SPELL_BIG_BANG, IMMUNITY_DAMAGE, SPELL_SCHOOL_MASK_NORMAL, false);
                //    }
                //}
                DoCast(me->getVictim(), SPELL_ASCEND);
            }
            ...
            if (!Phase2)
            {
                ...
                {
                    DoCast(me->getVictim(), RAID_MODE(H_SPELL_BIG_BANG, SPELL_BIG_BANG));
                    switch (rand()%2)
                    {
                    case 0: DoScriptText(SAY_BIGBANG_1, me); break;
                    case 1: DoScriptText(SAY_BIGBANG_2, me); break;
                    }
                    BigBang_Timer = 90000;
                ...
                //        AddsDespawn(BlackHole_GUID);
                    ...
                    for (ThreatList::const_iterator i = list.begin(); i != list.end(); ++i)
                    {
                        if (Unit* pU = Unit::GetUnit((*me), (*i)->getUnitGuid()))
                        {
                            ...
                            pU->ApplySpellImmune(RAID_MODE(H_SPELL_BIG_BANG, SPELL_BIG_BANG), IMMUNITY_ID, 0, false);
                        }
                    }
                }
                ...
                if (PhasePunch_Timer < diff)
                {
                   DoCast(me->getVictim(), SPELL_PHASE_PUNCH);
                    if (me->getVictim()->GetAura(SPELL_PHASE_PUNCH) && me->getVictim()->GetAura(SPELL_PHASE_PUNCH)->GetStackAmount() >= 5)
            ...
            if (CosmicSmash_Timer < diff)
            {
                ...
                else
                {
                    ...
                    {
                        DoCast(pT, RAID_MODE(SPELL_COSMIC_SMASH, H_SPELL_COSMIC_SMASH));
                        pT->ForcedDespawn();
                    }
                    if (me->GetMap()->IsHeroic())
                    {
                        ...
                        {
                            DoCast(pT, H_SPELL_COSMIC_SMASH);
                            pT->ForcedDespawn();
                        }
        ...
        void UpdateTexts(const uint32 diff)
        {
            if (IntroStart && !IntroEnd)
            {
                ...
                if (IntroTimer < diff)
                {
                    ...
                    case 4:
                        DoScriptText(SAY_FIRST_TIME, me);
                        IntroEnd = true;
                        DoStartCombat();
                        break;
                    }
            ...
            if (DespawnStart && !DespawnEnd)
            {
                if (DespawnTimer < diff)
                {
                    ...
                    {
                    case 1:
                        DoScriptText(SAY_DESPAWN_1, me);
                        DespawnTimer = 11000;
                        ++DespawnText;
                        break;
                    case 2:
                        DoScriptText(SAY_DESPAWN_2, me);
                        DespawnTimer = 8000;
                        ++DespawnText;
                        break;
                    case 3:
                        DoScriptText(SAY_DESPAWN_3, me);
                        DespawnTimer = 7000;
                        ++DespawnText;
                        break;
                    case 4:
            ...
            if (DefeatedStart &&  !DefeatedEnd)
            {
                if (DefeatedTimer < diff)
                {
                    ...
                    {
                    case 1:
                        DoScriptText(SAY_DEFEATED_1, me);
                        DefeatedTimer = 38000;
                        ++DefeatedText;
                        break;
                    case 2:
                        DoScriptText(SAY_DEFEATED_2, me);
                        DefeatedTimer = 16000;
                        ++DefeatedText;
                        break;
                    case 3:
                        DoScriptText(SAY_DEFEATED_3, me);
                        DefeatedTimer = 9000;
                        ++DefeatedText;
                        break;
                    case 4:
                        DoScriptText(SAY_DEFEATED_4, me);
                        DefeatedTimer = 10000;
                        ++DefeatedText;
                        break;
                    case 5:
                        DoScriptText(SAY_DEFEATED_5, me);
                        DefeatedTimer = 10000;
                        ++DefeatedText;
                        break;
                    case 6:
        ...
        {
            DoScriptText(SAY_SUMMONING_STAR, me);
            uint32 random1;
            uint32 random2;
            uint32 random3;
            uint32 random4;
                ...
                for (ThreatList::const_iterator i = list.begin(); i != list.end(); ++i)
                {
                    ...
                    if (isNearestBlackHole(pP))
                    {
                        ...
                        if (me->GetDistance2d(pP) < 5.0f)
                        {
                            if (itr != Timers.end()) // if 'find' find position
                            {
                                if (itr->second >= 2000)
                                {
                                    ...
                                    pP->ApplySpellImmune(0, IMMUNITY_ID, SPELL_BIG_BANG, true);
                        ...
                        else if (itr != Timers.end())
                        {
                            ...
                            if (!pP->HasAura(62168))
                            {
                                pP->ApplySpellImmune(0, IMMUNITY_ID, SPELL_BIG_BANG, false);
