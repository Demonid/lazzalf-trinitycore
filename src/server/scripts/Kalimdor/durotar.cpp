...
//                    { 
//                        attacker->AddAura(SPELL_SUMMON_ZENTABRA_TRIGGER, vehSummoner); 
...
//                    return; 
//                Talk(SAY_START); 
//                // Cant use StartFollow() here - would only make the followers to stack on eachother since we cant edit angle 
...
//            void SpellHit(Unit* caster, const SpellEntry* spell) 
//            { 
//                if (spell->Id == SPELL_AOE_TURNIN && caster->GetEntry() == NPC_URUZIN 
//                    && me->HasAura(SPELL_VOLUNTEER_AURA) && !complete) 
//                { 
